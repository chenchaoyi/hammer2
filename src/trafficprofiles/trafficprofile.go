...
func (p *Profile) initPushNotification() {
	//_HOST := "http://bullhorn.gree-apps.net" // production
	_HOST := "http://107.20.206.128" // production
	_GAME := "/modern-war"

	p.addAPIFunc(5, "POST", "REST",
		func() (string, string) {
			_user := strconv.Itoa(rand.Intn(1000000))
			//_token := strconv.Itoa(rand.Intn(1000000))
      _token := "AC58175932C97354328A86C3A8CE358DFFB4F594C50BAEF56A9EE2A2530B6A5D"
			return _HOST + "/v1" + _GAME + "/players/" + _user + "/devices", "{\"type\": \"ios\", \"token\": \"" + _token + "\"}"
		})
	...
	p.addAPIFunc(5, "POST", "REST",
		func() (string, string) {
			_user := strconv.Itoa(rand.Intn(1000000))
			//_token := strconv.Itoa(rand.Intn(1000000))
      _token := "APA91bH2tFGijghF1yJBbhZW5sVjiBJWNPIuQ_fXlsL3LA_gEJOIHvFcBpBTrTIOxeHOMxhP7wHJaVJ0phyuo2FZf44ShWJWrNGuEqdwAhl10PYSTWq44HkqN4X_o93AoXKwxYHmoRfVEgVTsJ-7ohXWuTiOEjDnFQ"
			return _HOST + "/v1" + _GAME + "/players/" + _user + "/devices", "{\"type\": \"android\", \"token\": \"" + _token + "\"}"
		})
}
